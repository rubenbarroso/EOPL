...
;;;;;;;;;;;;;;;; new cases for chap 6 ;;;;;;;;;;;;;;;;
      ...
      (new-object-exp (class-name rands)
        (check-abstract-class! class-name)
...
;''
(define check-abstract-class!
  (lambda (class-name)
    (let ((specifier (class->specifier (lookup-class class-name))))
      (newline)
      (if (eqv? specifier 'abstract)
          (eopl:error 'new-object
            "Abstract class ~s cannot be instantiated" class-name)))))
  ...
  (lambda (c-decl)
    (let ((super-name (class-decl->super-name c-decl)))
      (let ((field-ids  (append
            ...
            (specifier (class-decl->abstraction-specifier c-decl)))
        (add-to-class-env!
          (a-class
...
(define class->specifier
  (lambda (c-struct)
    (cases class c-struct
      (a-class (specifier class-name super-name field-length field-ids methods)
        specifier))))
