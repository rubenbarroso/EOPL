...
;We will make use of the instanceof operator introduced in
;exercise 5.11 to resolve the binary method problem for
;color points

;Here the binary method problem is described in detail:
;http://www.google.es/url?sa=t&source=web&cd=8&ved=0CF8QFjAH&url=http%3A%2F%2Fwww.cis.upenn.edu%2F~bcpierce%2Fpapers%2Fbinary.ps&rct=j&q=binary%20method%20problem%20equals&ei=ZVlTTva4E4qr8APQgPmNCw&usg=AFQjCNEDu1HPKfqq6KvmvVOtr3ALWXdrdA
...
;     let o1 = new point(3, 4)
;         o2 = new point(3, 4)
;     in send o1 similarpoints(o2)")
;1
;
; point with colorpoint
;> (run
;    "class point extends object
;       field x
;       field y
...
;       method get_color () color
;     let o1 = new point(3, 4)
;         o2 = new colorpoint(3, 4, 127)
;     in send o1 similarpoints(o2)")
;1
;
; colorpoint with colorpoint
;> (run
;    "class point extends object
;       field x
;       field y
...
;       method set_color (c) set color = c
;       method get_color () color
;     let o1 = new colorpoint(3, 4, 145)
;         o2 = new colorpoint(3, 4, 127)
;     in send o1 similarpoints(o2)")
;1

; Now we include similarpoints in the colorpoint class
...
;         then zero?(-(send pt get_color(), color))
;         else 0
;     let o1 = new colorpoint(3, 4, 145)
;         o2 = new colorpoint(3, 4, 127)
;     in send o1 similarpoints(o2)")
...
;         o2 = new point(3, 4)
;     in send o1 similarpoints(o2)")
;Error reported by find-method-and-apply:
;No method for name get_color

; We can fix it by makin g use of instanceof:
;> (run
;    "class point extends object
;       field x
;       field y
...
;              then zero?(-(send pt get_color(), color))
;              else 0
;         else super similarpoints(pt)
;     let o1 = new colorpoint(3, 4, 145)
;         o2 = new point(3, 4)
;     in send o1 similarpoints(o2)")
;1

The problem with the binay methods are described in Item 8
of Effective Java, 2nd Ed.:

"Transitivity - The third requirement of the equals contract says that
if one object is equal to a second and the second object is equal to
a third, then the first object must be equal to the third. Again, it's
not hard to imagine violating this component to its superclass. In other
words, the subclass adds a piece of information that affects equals
comparisons.
So what's the solution? It turns out that this is a fundamental problem
of equivalence relations in object-oriented languages. There is no way
to extend an instantiable class and add a value component while
preserving the equals contract, unless you are willing to forgo the
