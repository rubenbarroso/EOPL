...
(define class-decl->class-name
  (lambda (c-decl)
    (cases class-decl c-decl
      (a-class-decl (class-name super-name static-ids static-exps field-ids m-decls)
        class-name))))
...
;''
;As we can see, we use the empty environment
;to evaluate the expressions of the static
;variables
      ...
      (let ((field-ids  (append
        ...
        (add-to-class-env!
          (a-class
            ...
            field-ids
            (extend-env-refs
              (class-decl->static-ids c-decl)
              (make-rib (class-decl->static-exps c-decl))
              (empty-env))
  ...
  (lambda (exps)
    (let ((len (length exps)))
      (let ((vec (make-vector len)))
        (for-each
          (lambda (pos exp)
            (vector-set! vec pos (eval-expression
                                            exp
                                            (empty-env))))
          (iota len) exps)
        vec))))
...
;''
;We add the new rib containing the static fields
(define apply-method
      ...
      (eval-expression (method->body method)
        (extend-env
          ...
          (extend-env-refs
            ...
            (object->fields self)
            (class-name->static-fields host-name))))))
...
(define class->class-name
  (lambda (c-struct)
    (cases class c-struct
      (a-class (class-name super-name field-length field-ids static-fields methods)
        class-name))))
...
;''
(define class-name->static-fields
  (lambda (class-name)
    (if (eqv? class-name 'object) '()
      (class->static-fields (lookup-class class-name)))))
