...
;Changes code marked with ;''

;''
(define run
  (lambda (string)
    (eval-program (name-mangle (scan&parse string)))))
  ...
  (lambda (pgm)
    (cases program pgm
      (a-program (c-decls exp)
                 (a-program (name-mangle-class-decls c-decls)
                            (name-mangle-exp exp))))))

;''
(define name-mangle-class-decls
  (lambda (c-decls)
    (map name-mangle-class-decl c-decls)))

;''
(define name-mangle-class-decl
  (lambda (c-decl)
    (cases class-decl c-decl
      (a-class-decl (class-name super-name field-ids m-decls)
                    (a-class-decl class-name
                                  super-name
                                  field-ids
                                  (map method-name-mangle m-decls))))))

;''
(define method-name-mangle
  (lambda (m-decl)
    (cases method-decl m-decl
           (a-method-decl (method-name ids body)
                          (a-method-decl (string->symbol
                                           (string-append
  ...
  (lambda (exp)
    (cases expression exp
      ...
      (primapp-exp (prim rands)
                   (primapp-exp
                     prim
                     (name-mangle-rands rands)))
      (if-exp (test-exp true-exp false-exp)
              (if-exp (name-mangle-exp test-exp)
                      (name-mangle-exp true-exp)
                      (name-mangle-exp false-exp)))
      (let-exp (ids rands body)
               (let-exp
                 ids
                 (name-mangle-rands rands)
                 (name-mangle-exp body)))
      (proc-exp (ids body)
                (proc-exp
                  ids
                  (name-mangle-exp exp)))
      ...
      (letrec-exp (proc-names idss bodies letrec-body)
                  (letrec-exp
                    proc-names
                    idss
                    (name-mangle-rands bodies)
                    (name-mangle-exp letrec-body)))
      (varassign-exp (id rhs-exp)
                     (varassign-exp
                       id
                       (name-mangle-exp rhs-exp)))
      (begin-exp (exp1 exps)
                 (begin-exp
                   (name-mangle-exp exp1)
                   (name-mangle-rands exps)))

      (new-object-exp (class-name rands) exp)
      (method-app-exp (obj-exp method-name rands)
                      (method-app-exp
                        obj-exp
                        (method-call-name-mangle method-name rands)
                        (name-mangle-rands rands)))
...
;''
(define method-call-name-mangle
  (lambda (method-name rands)
    (string->symbol
      (string-append
        (symbol->string method-name)
        ":@"
...
;''
(define name-mangle-rands
  (lambda (exps)
    (map
      (lambda (exp) (name-mangle-exp exp))
      exps)))
      ...
      (new-object-exp (class-name rands)
        (let ((args (eval-rands rands env))
          ...
          (find-method-and-apply
            'initialize:@0 class-name obj args)
          obj))
      (method-app-exp (obj-exp method-name rands)
        (let ((args (eval-rands rands env))
