...
(let ((time-stamp "Time-stamp: <2001-05-10 13:13:24 dfried>"))
  (eopl:printf "4-2.scm: simple type checker ~a~%"
    (substring time-stamp 13 29)))

;;; Interpreter and type checker for simple type-checked language
...
;
;> (type-check
;    "let p = proc (int x, int y) +(x,y)
;     in (p 6 14)")
;int
;
;> (type-check
;    "let p = proc (int x, bool y) +(x,y)
;     in (p 6 14)")
...
;
;> (type-check
;    "letrec int fact (int x) = if zero?(x) then 1 else *(x,(fact sub1(x)))
;     in (fact 3)")
;int
;
;> (type-check
...
;Wrong number of arguments in expression (app-exp (var-exp fact) ((lit-exp 3) (lit-exp 5))):
;expected (int)
;got (int int)
