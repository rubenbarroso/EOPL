...
;''
(define pair-type->first
  (lambda (ty)
    (cases type ty
      (pair-type (first second) first)
      (else (eopl:error 'pair-type->first
              "Not a pair type: ~s" ty)))))

;''
(define pair-type->second
  (lambda (ty)
    (cases type ty
      (pair-type (first second) second)
      (else (eopl:error 'pair-type->second
              "Not a pair type: ~s" ty)))))
  ...
  (lambda (t1 t2 exp)
    (cond
      ...
      ((and (pair-type? t1) (pair-type? t2))
       (check-equal-type! (pair-type->first t1) (pair-type->first t2) exp)
       (check-equal-type! (pair-type->second t1) (pair-type->second t2) exp))
...
;(tvar1 |->| (< tvar1 : int >))
;> (type-check
;    "let p = proc(? x) pair(x,3)
;     in (p 3)")
;(< int : int >)
;> (type-check
;    "let p = proc(? t) unpack x y = t in +(x,y)
;     in (p 3)")
;int
