...
;Tests
;> (run "if eq?(1,1) then 5 else 6")
;6
;> (run "if eq?(emptylist,emptylist) then 5 else 6")
;5
;> (run "if eq?(i,x) then 5 else 6")
;6

;As we can see, eq? does not performs equality between numbers, only on
;symbols and lists. From SICP: "More generally, (eq? x y) tests whether
;x and y are the same object (that is, whether x and y are equal pointers)".
;It made no sense to test for eq? before introducing the list primitives
;because we had no support for symbols.
